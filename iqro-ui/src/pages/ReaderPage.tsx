...
interface ReaderPageProps {
  ...
  loadPageData: (levelId: number, pageId: number, ocrName: string) => Promise<IqroPageData>;
  ocrEngine: string;
...
export const ReaderPage: React.FC<ReaderPageProps> = ({
  ...
  useEffect(() => {
    ...
    const fetchPages = async () => {
      ...
      try {
        const rightData = await loadPageData(currentLevel.id, currentPageNumber, ocrEngine);
        setRightPageData(rightData);

        const nextPageNumber = currentPageNumber + 1;
        if (isDesktop && nextPageNumber <= currentLevel.pagesCount) {
          try {
            const leftData = await loadPageData(currentLevel.id, nextPageNumber, ocrEngine);
  ...
  if (!currentLevel || isNaN(parsedLevelId)) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-6 text-center bg-slate-50">
        <h2 className="text-xl font-bold text-red-600 mb-2">Jilid Iqro Tidak Ditemukan</h2>
        <p className="text-slate-600 text-sm mb-6">Pilihan jilid tidak valid atau belum tersedia.</p>
        <button
